    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 8, marginBottom: '1rem' }}>
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          <input style={{ width: '100%', padding: '8px 10px 8px 32px', fontSize: 13, border: busca ? '1px solid #185FA5' : '0.5px solid rgba(0,0,0,0.18)', borderRadius: 8, background: '#fff', color: '#111', outline: 'none', boxSizing: 'border-box' }} placeholder="Buscar por nome ou OAB..." value={busca} onChange={e => setBusca(e.target.value)} />
          {busca && <button onClick={() => setBusca('')} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, color: '#aaa' }}>×</button>}
        </div>
