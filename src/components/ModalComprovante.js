...
export default function ModalComprovante({ lote, onClose, onConfirm }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(255,255,255,0.05)', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div style={{ background: '#232a37', borderRadius: 16, padding: '1.5rem', width: '100%', maxWidth: 400 }}>
        ...
        {!arquivo ? (
          <div onClick={() => inputRef.current.click()} style={{ border: '1.5px dashed rgba(0,0,0,0.45)', borderRadius: 10, padding: '2rem', textAlign: 'center', cursor: 'pointer', background: '#171c26' }}>
          ...
          <div style={{ border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 10, overflow: 'hidden', marginBottom: 4 }}>
            {preview === 'pdf' ? (
              <div style={{ padding: '1.5rem', textAlign: 'center', background: '#171c26' }}>
                <div style={{ fontSize: 32, marginBottom: 6 }}>📄</div>
                <div style={{ fontSize: 13, color: '#e6edf7', fontWeight: 500 }}>{arquivo.name}</div>
              </div>
            ) : (
              <img src={preview} alt="Comprovante" style={{ width: '100%', maxHeight: 200, objectFit: 'contain', background: '#171c26' }} />
            )}
