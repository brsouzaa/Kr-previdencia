  ...
  return (
    <div>
      <div style={{ marginBottom: '1.5rem' }}>
        <div style={{ fontSize: 20, fontWeight: 500, color: '#e6edf7', marginBottom: 4 }}>🏆 Ranking de Vendedoras</div>
        <div style={{ fontSize: 13, color: '#8b9bb4' }}>Acompanhamento de produção e bônus mensal · atualiza a cada 60 segundos</div>
      </div>

      {/* === CARDS DE TOTAIS === */}
      <div style={s.cards}>
        <div style={s.cardMetric}>
          <div style={{ ...s.cardLabel, color: '#34d399' }}>Assinados hoje</div>
          <div style={{ ...s.cardValue, color: '#34d399' }}>{totalAssinadosHoje}</div>
          <div style={s.cardSub}>{ativosHoje} de {ranking.length} ativ{ativosHoje !== 1 ? 'as' : 'a'}</div>
        </div>
        <div style={s.cardMetric}>
          <div style={{ ...s.cardLabel, color: '#60a5fa' }}>Assinados na semana</div>
          <div style={{ ...s.cardValue, color: '#60a5fa' }}>{totalAssinadosSem}</div>
          <div style={s.cardSub}>últimos 7 dias</div>
        </div>
        <div style={s.cardMetric}>
          <div style={{ ...s.cardLabel, color: '#a78bfa' }}>Assinados no mês</div>
          <div style={{ ...s.cardValue, color: '#a78bfa' }}>{totalAssinadosMes}</div>
          <div style={s.cardSub}>mês corrente</div>
        </div>
        <div style={s.cardMetric}>
          <div style={{ ...s.cardLabel, color: '#fbbf24' }}>
            💰 Bônus {periodo === 'custom' && rankingCustom ? 'do período' : 'do mês'}
          </div>
          <div style={{ ...s.cardValue, color: '#fbbf24' }}>
      ...
      {(vendedoresEmQueda.length > 0 || zeraramHoje.length > 0) && (
        <div style={s.block}>
          ...
          {vendedoresEmQueda.length > 0 && (
            <div style={s.alertBox('#f87171', 'rgba(248,113,113,.14)')}>
          ...
          {zeraramHoje.length > 0 && zeraramHoje.length < ranking.length && (
            <div style={s.alertBox('#fbbf24', 'rgba(251,191,36,.12)')}>
        ...
        {rankingOrdenado.map(r => {
          ...
          return (
            <div key={r.vendedor_id} style={s.rankRow}>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 500, color: '#e6edf7' }}>{r.vendedor_nome}</div>
                <div style={{ fontSize: 10, color: '#8b9bb4' }}>
                  {cadastros} cadastros · {tempoMedio ? `${Math.round(tempoMedio)}h média` : 'sem dados'}
                </div>
              </div>
              <div style={{ textAlign: 'right', fontSize: 14, fontWeight: 500, color: '#60a5fa' }}>
                {qtd}
              </div>
              <div style={{ textAlign: 'right', fontSize: 12, color: taxa >= 60 ? '#34d399' : taxa >= 40 ? '#fbbf24' : '#f87171' }}>
                {taxa}%
              </div>
              <div style={{ textAlign: 'right', fontSize: 13, fontWeight: 500, color: bonus > 0 ? '#34d399' : '#64748b' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(148,163,184,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#131e33', padding: 24, borderRadius: 12, width: '90%', maxWidth: 440 }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 12, color: '#e6edf7', fontWeight: 500, marginBottom: 4, display: 'block' }}>Data início *</label>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 12, color: '#e6edf7', fontWeight: 500, marginBottom: 4, display: 'block' }}>Data fim *</label>
              <input type="date" value={dataFim} onChange={e => setDataFim(e.target.value)}
                style={{ width: '100%', padding: 10, fontSize: 14, border: '0.5px solid rgba(0,0,0,0.45)', borderRadius: 8, outline: 'none', boxSizing: 'border-box' }} />
            </div>

            <div style={{ background: 'rgba(52,211,153,.14)', padding: 10, borderRadius: 6, fontSize: 11, color: '#34d399', marginBottom: 14 }}>
