...
export default function SupervisorProducao() {
  ...
  return (
    <div>
      <div style={{ fontSize: 20, fontWeight: 500, color: '#111', marginBottom: '1.25rem' }}>📊 Supervisão de Produção</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: '1.25rem' }}>
        ...
        <div style={card}>
          ...
          {[
            ['Assinados', assinados, '#3B6D11', '#EAF3DE'],
            ['Aguardando assinatura', pendentes, '#854F0B', '#FAEEDA'],
          ].map(([l, v, c, bg]) => {
          ...
          <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '0.5px solid rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: 12 }}>
              <span style={{ padding: '3px 8px', borderRadius: 20, fontSize: 11, fontWeight: 500,
                background: c.status === 'assinado' ? '#EAF3DE' : '#FAEEDA',
                color: c.status === 'assinado' ? '#3B6D11' : '#854F0B' }}>
