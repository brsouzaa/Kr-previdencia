...
const s = {
  ...
  card: (cor) => ({ border: '0.5px solid rgba(0,0,0,0.08)', borderLeft: `4px solid ${cor}`, borderRadius: 10, padding: '10px 12px', background: '#fff' }),
  cardTop: { fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.4px', fontWeight: 600, color: '#888', marginBottom: 4 },
  ...
  aba: (on) => ({ padding: '7px 14px', borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: 'pointer', border: '1px solid ' + (on ? '#185FA5' : 'rgba(0,0,0,0.12)'), background: on ? '#185FA5' : '#fff', color: on ? '#fff' : '#444' }),
  box: { border: '0.5px solid rgba(0,0,0,0.1)', borderRadius: 12, padding: 14, marginBottom: 12, background: '#fff' },
  boxTitulo: { fontSize: 14, fontWeight: 600, marginBottom: 10 },
  btn: { padding: '8px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', border: 'none', background: '#185FA5', color: '#fff', marginRight: 6 },
  btnG: { padding: '8px 14px', borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: 'pointer', border: '1px solid rgba(0,0,0,0.15)', background: '#fff', color: '#444', marginRight: 6 },
  btnVerde: { padding: '8px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', border: 'none', background: '#256B2E', color: '#fff', marginRight: 6 },
  btnVermelho: { padding: '8px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', border: 'none', background: '#A32D2D', color: '#fff', marginRight: 6 },
  input: { padding: '8px 10px', borderRadius: 8, border: '1px solid rgba(0,0,0,0.15)', fontSize: 13, marginRight: 6 },
  tag: (bg, cor) => ({ fontSize: 11, fontWeight: 700, background: bg, color: cor, borderRadius: 6, padding: '2px 8px', display: 'inline-block' }),
  linha: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '10px 0', borderBottom: '0.5px solid rgba(0,0,0,0.06)', flexWrap: 'wrap' },
  mono: { fontFamily: 'monospace', fontSize: 12 },
  toggleGrande: (on) => ({ padding: '14px 28px', borderRadius: 12, fontSize: 16, fontWeight: 700, cursor: 'pointer', border: 'none', background: on ? '#256B2E' : '#A32D2D', color: '#fff' }),
  campoGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px,1fr))', gap: 6, marginTop: 8 },
  campo: { fontSize: 12, background: '#F8F9FA', borderRadius: 6, padding: '5px 8px' },
  logRow: (nivel) => ({ fontSize: 12, fontFamily: 'monospace', padding: '5px 8px', borderRadius: 6, marginBottom: 3, background: nivel === 'erro' ? '#FCEBEB' : '#F8F9FA', color: nivel === 'erro' ? '#A32D2D' : '#444' }),
  printImg: { maxWidth: '100%', borderRadius: 8, border: '0.5px solid rgba(0,0,0,0.15)', cursor: 'pointer' },
  chip: (on) => ({ padding: '5px 12px', borderRadius: 999, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: '1px solid ' + (on ? '#185FA5' : 'rgba(0,0,0,0.15)'), background: on ? '#E6F1FB' : '#fff', color: on ? '#185FA5' : '#666', marginRight: 6 }),
}

const STATUS_SESSAO = {
  ativa: ['#EAF3DE', '#3B6D11'], caida: ['#FCEBEB', '#A32D2D'],
  expirada: ['#FAEEDA', '#854F0B'], logando: ['#E6F1FB', '#185FA5'],
}
...
export default function PainelDigitador() {
  ...
  const marcarRelogin = async (usuario) => {
    await supabase.from('digitador_sessoes').update({ precisa_relogin: true, atualizado_em: new Date().toISOString() }).eq('usuario', usuario)
    carregar()
  }
  ...
  return (
    <div style={s.wrap}>
      ...
      {/* SAÚDE */}
      <div style={s.cards}>
        <div style={s.card(vivo ? '#256B2E' : '#A32D2D')}>
          <div style={s.cardTop}>Robô</div>
          <div style={{ ...s.cardNum, color: vivo ? '#256B2E' : '#A32D2D' }}>{vivo ? 'VIVO' : 'SEM SINAL'}</div>
          <div style={s.cardSub}>{heartbeat ? `ping ${fmtTs(heartbeat.ultimo_ping)} · ${heartbeat.versao || ''}` : 'nunca pingou'}</div>
        </div>
        <div style={s.card(control?.ligado ? '#256B2E' : '#A32D2D')}>
          <div style={s.cardTop}>Fila</div>
          <div style={{ ...s.cardNum, color: control?.ligado ? '#256B2E' : '#A32D2D' }}>{control?.ligado ? 'LIGADA' : 'PAUSADA'}</div>
          <div style={s.cardSub}>modo: {control?.modo || '—'}{!janela && ' · FORA DA JANELA'}</div>
        </div>
        <div style={s.card('#185FA5')}>
        ...
        <div style={s.card(conf.taxa_acerto_pct == null ? '#888' : conf.taxa_acerto_pct >= 95 ? '#256B2E' : '#C88A00')}>
        ...
        <div style={s.card(caidas.length ? '#A32D2D' : '#256B2E')}>
          <div style={s.cardTop}>Logins</div>
          <div style={{ ...s.cardNum, color: caidas.length ? '#A32D2D' : '#256B2E' }}>{sessoes.length - caidas.length}/{sessoes.length} ok</div>
      ...
      {aba === 'conferencia' && (
        <div>
          ...
          {confs.length === 0 && (
            <div style={{ ...s.box, color: '#999', textAlign: 'center' }}>
            ...
            <div key={reg.id} style={s.box}>
              <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                <div>
                  ...
                  {' '}<span style={reg.modo === 'producao' ? s.tag('#EAF3DE', '#3B6D11') : s.tag('#E6F1FB', '#185FA5')}>{reg.modo === 'producao' ? 'ENVIADA' : 'SOMBRA'}</span>
                  {reg.protocolo && <span style={{ ...s.tag('#F4F8FC', '#185FA5'), marginLeft: 6 }}>protocolo {reg.protocolo}</span>}
                  {' '}<span style={s.tag('#F8F9FA', '#666')}>RG: {reg.anexos_rg ?? 0} · Extrato: {reg.anexos_extrato ?? 0}</span>
                  {reg.usuario && <span style={{ ...s.tag('#F8F9FA', '#666'), marginLeft: 6 }}>{reg.usuario}</span>}
                  {reg.conferido && <span style={{ ...s.tag('#EAF3DE', '#3B6D11'), marginLeft: 6 }}>✓ correto ({reg.conferido_por})</span>}
                  {reg.problema && <span style={{ ...s.tag('#FCEBEB', '#A32D2D'), marginLeft: 6 }}>⚠ problema ({reg.conferido_por})</span>}
                </div>
              ...
              {reg.obs && <div style={{ fontSize: 12, color: '#A32D2D', marginTop: 6 }}>Obs: {reg.obs}</div>}
              {reg.erro_detalhe && <div style={{ fontSize: 12, color: '#A32D2D', marginTop: 6 }}>Erro: {reg.erro_detalhe}</div>}
              {reg.campos_json && (
              ...
              {prints[reg.id] && (
                <div style={{ marginTop: 10 }}>
                  ...
                  <div style={{ fontSize: 11, color: '#888' }}>clique pra abrir em tamanho real</div>
                </div>
              )}
            </div>
          ))}
      ...
      {aba === 'controle' && control && (
        <div style={s.box}>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ fontSize: 12, color: '#888', marginTop: 4 }}>Produção é o modo normal. Sombra é só pra teste quando você quiser conferir antes — não é etapa do fluxo.</div>
          </div>
          ...
          <div style={{ fontSize: 12, color: '#888', marginTop: 10 }}>Última alteração: {fmtTs(control.atualizado_em)} por {control.atualizado_por || '—'}</div>
        </div>
      )}
      ...
      {aba === 'logins' && (
        <div>
          <div style={s.box}>
            ...
            <div style={{ fontSize: 11, color: '#888', marginTop: 6 }}>A senha vai cifrada direto pro banco via RPC — não aparece em log nem volta pro painel. Pra trocar, salve de novo com o mesmo usuário.</div>
          </div>
          <div style={s.box}>
            <div style={s.boxTitulo}>Logins e rodízio</div>
            {sessoes.length === 0 && <div style={{ color: '#999', fontSize: 13 }}>Nenhum login cadastrado.</div>}
            {sessoes.map(x => {
              const [bg, cor] = STATUS_SESSAO[x.status] || ['#F4F5F7', '#555']
              return (
                <div key={x.usuario} style={s.linha}>
                  <div>
                    <strong style={s.mono}>{x.usuario}</strong>{' '}
                    <span style={s.tag(bg, cor)}>{x.status}</span>
                    {x.precisa_relogin && <span style={{ ...s.tag('#FAEEDA', '#854F0B'), marginLeft: 6 }}>precisa relogin</span>}
                    <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>último OK: {fmtTs(x.ultimo_ok_em)}{x.ultimo_erro ? ` · erro: ${x.ultimo_erro}` : ''}</div>
                  </div>
                  <div>
          ...
          {vncAberto && (
            <div style={s.box}>
              ...
              {control?.vnc_url
                ? <iframe title="vnc" src={control.vnc_url} style={{ width: '100%', height: 560, border: '0.5px solid rgba(0,0,0,0.15)', borderRadius: 8 }} />
                : <div style={{ fontSize: 13, color: '#A32D2D' }}>URL do VNC não configurada — preencha na aba Controle quando o VPS estiver no ar.</div>}
              <div style={{ fontSize: 12, color: '#888', marginTop: 6 }}>Depois do login, o robô re-salva a sessão sozinho e o status volta pra "ativa".</div>
            </div>
          )}
        </div>
      )}
      ...
      {aba === 'erros' && (
        <div>
          <div style={s.box}>
            ...
            {problemas.length === 0 && <div style={{ color: '#999', fontSize: 13 }}>Nenhum lead com erro/revisão. 🎉</div>}
            {problemas.map(p => (
              <div key={p.lead_id} style={s.linha}>
                <div>
                  ...
                  <div style={{ fontSize: 12, marginTop: 2 }}>
                    <span style={s.tag('#FCEBEB', '#A32D2D')}>{p.status}</span>{' '}
                    {p.detalhe && <span style={{ color: '#666' }}>{p.detalhe}</span>}
                  </div>
                </div>
                <div style={{ fontSize: 11, color: '#888' }}>{p.digitado_em ? fmtTs(p.digitado_em) : ''} {p.digitado_por ? `· ${p.digitado_por}` : ''}</div>
              </div>
            ))}
          </div>
          <div style={s.box}>
            ...
            {logs.length === 0 && <div style={{ color: '#999', fontSize: 13 }}>Sem logs ainda (o robô grava aqui a cada evento).</div>}
            {logs.map(l => (
        ...
        <div style={s.box}>
          ...
          {resumo?.bloqueados_por_motivo && Object.keys(resumo.bloqueados_por_motivo).length > 0
            ? Object.entries(resumo.bloqueados_por_motivo).sort((a, b) => b[1] - a[1]).map(([m, q]) => (
                <div key={m} style={{ fontSize: 13, padding: '4px 0', borderBottom: '0.5px solid rgba(0,0,0,0.05)' }}>
                  <strong>{q}</strong> — {m}
                </div>
              ))
            : <div style={{ color: '#999', fontSize: 13 }}>Nenhum bloqueado agora.</div>}
        </div>
      )}
    </div>
  )
