...
const VERDE_ESCURO = '#059669', VERDE = '#059669', LARANJA = '#b45309', VERMELHO = '#dc2626', AZUL = '#2563eb', ROXO = '#7c3aed'
...
export default function MetasFinanceiras() {
  ...
  if (!podeVer) return <div style={{ padding: '3rem', textAlign: 'center', color: '#5b6b84' }}>Acesso restrito à administração.</div>
  if (loading) return <div style={{ padding: '3rem', textAlign: 'center', color: '#5b6b84' }}>Carregando metas…</div>
  ...
  const reguas = metaMes ? [
    ...
    { nome: 'Imposto', gasto: gastoImp, niveis: [null, null, Number(metaMes.teto_imposto_pct)], cor: '#5b6b84' },
    { nome: 'Variáveis (resto)', gasto: gastoVar, niveis: [null, null, Number(metaMes.teto_variavel_pct)], cor: '#059669' },
  ].map(r => {
  ...
  const card = { background: '#ffffff', border: '0.5px solid rgba(15,23,42,0.08)', borderRadius: 14, padding: '1rem 1.25rem' }

  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 0 40px' }}>
      <h2 style={{ fontSize: 22, fontWeight: 700, color: '#0f172a', margin: '0 0 2px' }}>🥅 Metas & Saúde Financeira</h2>
        ...
        {metas.map(m => {
          ...
          return (
            <div key={m.mes} style={{
              ...
              background: ehAtual ? 'rgba(96,165,250,.10)' : '#ffffff',
            }}>
              <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: ehAtual ? AZUL : '#5b6b84' }}>
                {MES_CURTO[d.getMonth()]}/{String(d.getFullYear()).slice(2)}{ehAtual ? ' • agora' : ''}
              </div>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', marginTop: 2 }}>{fmtK(m.meta_faturamento)}</div>
              {(ehAtual || passado) && (
      ...
      {!metaMes ? (
        <div style={{ ...card, textAlign: 'center', color: '#5b6b84', fontSize: 13 }}>Sem meta cadastrada pro mês corrente.</div>
      ) : (
        <>
          {/* MÊS CORRENTE */}
          <div style={{ ...card, marginBottom: 14 }}>
            ...
            <div style={{ height: 14, background: 'rgba(15,23,42,0.05)', borderRadius: 8, overflow: 'hidden', position: 'relative' }}>
              <div style={{ width: `${Math.min(metaFat > 0 ? receita / metaFat * 100 : 0, 100)}%`, height: '100%', background: VERDE, borderRadius: 8 }} />
              <div style={{ position: 'absolute', left: `${Math.min(fracao * 100, 100)}%`, top: 0, bottom: 0, width: 2, background: '#0f172a', opacity: 0.5 }} />
            </div>
          </div>
          ...
          <div style={{ ...card, marginBottom: 14 }}>
            ...
            <div style={{ fontSize: 11, color: '#5b6b84', margin: '2px 0 12px' }}>
              ...
              <div key={r.nome} style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12.5, marginBottom: 3, flexWrap: 'wrap', gap: 4 }}>
                  ...
                  <span style={{ color: '#5b6b84' }}>
                    ...
                    {r.saud != null && <span style={{ color: '#5b6b84' }}> · níveis {r.saud}/{r.alvo}/{r.tol}%</span>}
                    {r.saud == null && <span style={{ color: '#5b6b84' }}> · teto {r.tol}%</span>}
                  </span>
                </div>
                <div style={{ height: 10, background: 'rgba(15,23,42,0.05)', borderRadius: 6, overflow: 'hidden', position: 'relative' }}>
                ...
                <div style={{ fontSize: 11, color: '#5b6b84', marginTop: 2 }}>
                  teto do mês: {fmtK(r.tolRS)} · projeção no ritmo atual: <b style={{ color: r.projGasto > r.tolRS ? VERMELHO : '#5b6b84' }}>{fmtK(r.projGasto)}</b>
                  {r.projGasto > r.tolRS && ' ⚠ estoura no ritmo atual'}
                </div>
              </div>
            ))}
            <div style={{ fontSize: 11, color: '#5b6b84', borderTop: '0.5px solid rgba(0,0,0,0.07)', paddingTop: 8 }}>
          ...
          <div style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
              ...
              <button onClick={pedirAnaliseIA} disabled={iaLoading}
                style={{ fontSize: 12.5, fontWeight: 600, padding: '7px 14px', borderRadius: 8, border: `1px solid ${ROXO}`, background: iaLoading ? 'rgba(167,139,250,.14)' : '#ffffff', color: ROXO, cursor: 'pointer' }}>
                {iaLoading ? 'Analisando…' : ia ? '↻ Analisar de novo' : 'Gerar análise'}
              </button>
            </div>
            {ia ? (
              <div style={{ whiteSpace: 'pre-wrap', fontSize: 13, color: '#334155', marginTop: 10, lineHeight: 1.6, background: '#f1f5f9', borderRadius: 10, padding: '12px 14px' }}>{ia}</div>
            ) : (
